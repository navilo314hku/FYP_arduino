...
//sampling var start
int SAMPLE_SIZE=22;
int writing_count=0; //0=not writing  >0=writing 
//sampling var end
//callibration variables start
int start_button_count = 0;
int CALIBRATION_SAMPLE_SIZE = 10;
int CALIBRATION_FREQENCY = 40;
int calibrating = 0;
...
void autoCalibrate(int start_button_count, int main_loop_count,int CALIBRATION_FREQENCY){
  //if not writing 
  //TODO: modify the not writing condition
  if ( writing_count==0 & main_loop_count%CALIBRATION_FREQENCY==0 ){//not writing and reach calibration period
    calibrating=1;//Start calibration
    Serial.println("autoCalibrate"); 
  }
}
...
void ButtonsInput() {
  ...
  if (writing_count>SAMPLE_SIZE){//addition +3 for safety 
    writing_count=0; //notify the sampling process end
  }
  // save the last state
  ...
  if (saveButtonLastState == LOW && saveButtonCurrentState == HIGH) { //detect save button
    ...
    Serial.println("Manual Calibrating");
  }
